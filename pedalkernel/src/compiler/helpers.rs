...
/// Convert SP tree to DynNode with VoltageSource and CathodeBiasSource leaves.
///
/// Like `sp_to_dyn_with_vs`, but also replaces leaf components that terminate
/// at supply nodes with `CathodeBiasSource { voltage, rp }`. The
/// `supply_leaf_voltages` map is keyed by component index → supply voltage.
pub(super) fn sp_to_dyn_with_vs_and_supplies(
    tree: &SpTree,
    components: &[ComponentDef],
    fork_paths: &HashMap<usize, ForkPathInfo>,
    sample_rate: f64,
    vs_idx: usize,
    supply_leaf_voltages: &HashMap<usize, f64>,
) -> DynNode {
    match tree {
        SpTree::Leaf(idx) if *idx == vs_idx => DynNode::VoltageSource {
            voltage: 0.0,
        ...
        SpTree::Leaf(idx) if supply_leaf_voltages.contains_key(idx) => {
