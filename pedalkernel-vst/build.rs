...
fn main() {
    ...
    // Pro pedals are now in pedals/real/ subdirectory
    let pro_pedals_dir = PathBuf::from(&manifest_dir)
        .join("../../pedalkernel-pro/pedals/real");
    // Original/new designs in pedals/new/
    let pro_new_dir = PathBuf::from(&manifest_dir)
        .join("../../pedalkernel-pro/pedals/new");
    // Some pro pedals remain at top level (gate, tremolo, synth)
    let pro_pedals_top_dir = PathBuf::from(&manifest_dir)
    ...
    let pro_new_exists = pro_new_dir.exists();
    let pro_top_exists = pro_pedals_top_dir.exists();
    ...
    for (exists, pedals_dir) in [(pro_exists, &pro_pedals_dir), (pro_new_exists, &pro_new_dir), (pro_top_exists, &pro_pedals_top_dir), (main_exists, &main_pedals_dir), (main_synths_exists, &main_synths_dir)] {
        if exists {
    ...
    if !pro_exists && !pro_new_exists && !pro_top_exists && !main_exists && !main_synths_exists {
        ...
        let shared_dirs = ["compressor", "chorus", "phaser", "tremolo", "delay", "reverb", "modulation", "gate", "filter", "tone", "preamp"];

        // Collect from pro (pedals/real/, pedals/new/) and main repos
        let pedal_sources: Vec<&PathBuf> = [(pro_exists, &pro_pedals_dir), (pro_new_exists, &pro_new_dir), (main_exists, &main_pedals_dir)]
            .iter()
        ...
        // Also collect from pro top-level directories (gate, tremolo, synth, phaser)
        // These remain at pedals/ level, not in pedals/real/
        if pro_top_exists {
            for dir_name in &synth_dirs {
                collect_pedals(&pro_pedals_top_dir.join(dir_name), &mut synth_pedals);
            }
            for dir_name in &shared_dirs {
                collect_pedals(&pro_pedals_top_dir.join(dir_name), &mut shared_pedals);
            }
        }
