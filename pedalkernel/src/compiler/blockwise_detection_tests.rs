//! TDD tests for blockwise decomposition detection.
//!
//! These tests define circuit topologies that should be decomposed into
//! chained WDF blocks (blockwise method) instead of one monolithic R-type
//! adaptor. Each test constructs the circuit, runs the SPQR decomposition
//! (or the full compile pipeline), and asserts the structural conditions
//! that the future blockwise optimizer will detect.
//!
//! All tests are expected to FAIL until blockwise decomposition is implemented.
//! They define the target behavior through assertions.
