        ...
        match &info.feedback_kind {
            ...
            OpAmpFeedbackKind::Inverting {
                ...
                if !skip_tree || feedback_diode.is_none() {
                    // For skip_tree opamps (output adjacent to NL junction via a series
                    // resistor, e.g. RAT R3), exclude any RESISTOR that directly connects
                    // out_node to the NL junction. That resistor belongs to the diode
                    // stage as a port termination, not to the opamp feedback tree.
                    // Capacitors (e.g. C3 frequency compensation) are always kept.
                    let ids_to_claim: Vec<String> = if skip_tree {
                        let out = info.out_node;
                        info.feedback_comp_ids.iter().filter(|id| {
                            // Keep if NOT a resistor directly connecting out_node to some other node.
                            // Check: does this comp ID correspond to an edge with out_node as endpoint
                            // where the component is a resistor (not a capacitor/inductor)?
                            let edges_for_id = comp_ids_to_edge_indices(
                                std::slice::from_ref(*id)
                            );
                            let is_output_side_resistor = edges_for_id.iter().any(|&eidx| {
                                let e = &graph.edges[eidx];
                                let touches_out = e.node_a == out || e.node_b == out;
                                let comp = &graph.components[e.comp_idx];
                                let is_resistor = comp.kind.resistance().is_some()
                                    && comp.kind.capacitance().is_none();
                                touches_out && is_resistor
                            });
                            !is_output_side_resistor
                        }).cloned().collect()
                    } else {
                        info.feedback_comp_ids.clone()
                    };
                    consumed_edges.extend(comp_ids_to_edge_indices(&ids_to_claim));
                }
                stages.push(stage);
            }
