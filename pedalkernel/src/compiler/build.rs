...
pub(super) fn build_stages(
    ...
    let _unity_gain_out_nodes: Vec<super::graph::NodeId> = opamp_analysis
        .feedback_loops
        .iter()
...
fn stage_plan_to_multi_nl(
    ...
    let (nl_terminals, passive_edges) = match &elem.kind {
        ...
        NonlinearKind::SingleDiode(_) | NonlinearKind::DiodePair(_) => {
            ...
            // Use the plan's already-collected passive set (which respects
            // claimed_edges and barrier exclusion from plan_single_nl).
            // A fresh unconstrained BFS would pick up opamp feedback components
            // already claimed by the opamp stage (e.g. RAT C2/C3/Distortion/R4).
            let edges = plan.passive_idxs.clone();
    ...
    // If any passive edge touches graph.out_node, set output_node so the
    // MNA stage probes voltage at out_node (after tone/volume network).
    // Without this, the output is read from the NL junction itself (before
    // the volume pot), causing Volume=0 to still produce full output.
    let output_node = if passive_edges.iter().any(|&eidx| {
        let e = &graph.edges[eidx];
            ...
            Some(mut stage) => {
                ...
                if has_diode {
                    let all_junction_nodes: Vec<super::graph::NodeId> = plan
                        ...
                        .iter()
                        .flat_map(|&idx| {
                            classified.nonlinear_elements[idx].junction_nodes.iter().copied()
                        })
                        .collect();
                    if let Some(dp) = diode_paired_opamps.iter().find(|dp| {
