...
fn main() {
    ...
    let pedals = [
        (
            "RATKING",
            "ratking.pedal",
            vec![("Distortion", 0.5), ("Filter", 0.5), ("Volume", 0.5)],
        ),
        (
            "SCREAMER",
            "screamer.pedal",
            vec![("Drive", 0.5), ("Tone", 0.5), ("Level", 0.5)],
        ),
        (
            "GOLDENROD",
            "goldenrod.pedal",
            vec![("Gain", 0.5), ("Treble", 0.5), ("Output", 0.5)],
        ),
        (
            "MUFF",
            "muff.pedal",
            vec![("Sustain", 0.7), ("Tone", 0.5), ("Volume", 0.6)],
        ),
    ];

    let input: Vec<f64> = (0..4800)
        .map(|i| (2.0 * std::f64::consts::PI * 440.0 * i as f64 / 48000.0).sin() * 0.1)
        .collect();
    ...
    for (name, file, knobs) in &pedals {
        ...
        let rms =
            (output[480..].iter().map(|x| x * x).sum::<f64>() / (output.len() - 480) as f64).sqrt();
        println!(
            "{}: peak={:.6}, rms={:.6}, gain_dB={:.1}",
            name,
            peak,
            rms,
            20.0 * (peak / 0.1).log10()
    ...
    let rms =
        (output[480..].iter().map(|x| x * x).sum::<f64>() / (output.len() - 480) as f64).sqrt();
    println!(
        "proco_rat(core): peak={:.6}, rms={:.6}, gain_dB={:.1}",
        peak,
        rms,
        20.0 * (peak / 0.1).log10()
    );
