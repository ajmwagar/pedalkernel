...
pub(super) fn plan_stages(
    ...
    let (island_depths, node_island_depths) =
        compute_island_depths(&mut analysis, graph, classified);
    ...
    for group in &groups {
        ...
        if bjt_members.len() >= 2 {
            ...
            let member_indices: Vec<usize> = bjt_members
                .iter()
                .filter_map(|&elem_idx| bjt_terminals.iter().position(|b| b.elem_idx == elem_idx))
                .collect();
    ...
    let bjt_emitter_nodes: Vec<NodeId> = bjt_terminals.iter().map(|b| b.emitter_node).collect();
    ...
    for group in &groups {
        ...
        if active_members.len() == 1 {
            ...
            if let Some(plan) =
                try_varimu_3port(elem, elem_idx, classified, graph, &pp_transformer_edges)
            {
...
fn plan_one_junction(
    ...
    let other_terminal = if nl_edge.node_a == junction {
        nl_edge.node_b
    } else {
        nl_edge.node_a
    };
...
fn plan_two_junction(
    ...
    let (virtual_edge, compensation, dc_block) =
        match &elem.kind {
            NonlinearKind::Triode {
                model_name,
                is_vari_mu,
    ...
    let (load_passive_idxs, load_terminals) = match &elem.kind {
        NonlinearKind::BjtNpn { emitter_node, .. } | NonlinearKind::BjtPnp { emitter_node, .. } => {
...
fn plan_multi_nl_group(
    ...
    if !boundary_edges.is_empty() {
        ...
        let comp_names: Vec<String> = all_passive_edges
            ...
            .collect();
        let elem_names: Vec<String> = ordered
            .iter()
            .map(|&idx| {
                graph.components[graph.edges[classified.nonlinear_elements[idx].edge_idx].comp_idx]
                    .id
                    .clone()
            })
            .collect();
    ...
    Some(MultiNlPlan {
        ...
        nl_terminals,
        compensation: 1.0,
        output_node: if !output_tail.is_empty() {
            Some(graph.out_node)
        } else {
            None
        },
...
fn try_varimu_3port(
    ...
    let junction_nodes: HashSet<NodeId> = [*grid_node, *plate_node, *cathode_node]
        .into_iter()
        .collect();
    ...
    let exclude: HashSet<NodeId> = [*plate_node, *cathode_node, *grid_node]
        .into_iter()
        .collect();
...
fn try_bjt_two_port(
    ...
    let junction_nodes: HashSet<NodeId> = [base_node, collector_node, emitter_node]
        .into_iter()
        .collect();
    let mut all_passive_edges = collect_passive_edges_from_nodes(
        &junction_nodes,
    ...
    let exclude: HashSet<NodeId> = [base_node, collector_node, emitter_node]
        .into_iter()
        .collect();
    ...
    let injection_node = match boundary_injection {
        Some(bi) => {
            ...
            if fb_is_hub {
                ...
                let bi_dist = classified
                    .dist_from_in
                    .get(&bi)
                    .copied()
                    .unwrap_or(usize::MAX);
                let fb_dist = classified
                    .dist_from_in
                    .get(&fallback_injection)
                    .copied()
                    .unwrap_or(usize::MAX);
                if fb_dist < bi_dist {
                    fallback_injection
...
fn try_linearized_ota(
    ...
    let junction_nodes: HashSet<NodeId> = [pos_node, neg_node, out_node].into_iter().collect();
    let all_passive_edges = collect_passive_edges_from_nodes(
        &junction_nodes,
        graph,
        classified,
...
fn plan_diode_bridge(
    ...
    let active_pin_nodes: HashSet<NodeId> = graph
        ...
        .collect();
    // Unity-gain opamp pins (excluded from opamp_input_nodes) are transparent
    // wires and must not block BFS. Without this, passive tone networks
    // behind unity buffers are orphaned (e.g. SD-1 Tone__wb + C_t1 behind U2).
    // Keep only: (a) confirmed opamp input barriers, (b) non-opamp transistor
    // input pins, (c) nodes not on any opamp at all.
...
pub(super) fn plan_push_pull_half(
...

    {
        ...
        let exclude: HashSet<NodeId> = [plate_node, cathode_node].into_iter().collect();
        ...
        let mut ground_terminal = graph.gnd_node;
        {
            ...
            for (&node, &deg) in &degree {
                if deg == 1 && node != plate_node && node != cathode_node && node != injection_node
                {
