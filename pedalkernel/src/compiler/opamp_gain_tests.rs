...
/// Helper: iterate over only the WdfStage variants in a compiled pedal.
fn wdf_stages(proc: &super::compiled::CompiledPedal) -> impl Iterator<Item = &WdfStage> {
    proc.stages.iter().filter_map(|s| s.as_wdf())
}
...
/// Verify that the compiled RATKING has an opamp gain stage.
/// On the SPQR-tree branch, RATKING compiles into a BlackFeedbackStage (not WDF feedback_opamp).
#[test]
fn feedback_opamp_stage_exists_on_ratking() {
    ...
    for (i, s) in proc.stages.iter().enumerate() {
        if let Stage::Wdf(w) = s {
            ...
            eprintln!(
                "  stage[{i}]: Wdf root={root_tag} feedback_opamp={} feedback_pot={:?} opamp_adaptor={} opamp_wdf_adaptor={}",
                w.feedback_opamp.is_some(),
                w.feedback_pot_id,
    ...
    assert!(
        has_feedback_opamp || has_black_feedback,
        "RATKING should have at least one stage with feedback_opamp or BlackFeedback"
    );
}

/// Verify the Screamer compiles into stages with opamp gain processing.
/// On the SPQR-tree branch, the Screamer uses feedback_opamp (not OpAmpRoot).
#[test]
fn screamer_compiles_with_opamp_gain_stage() {
    ...
    for (i, s) in proc.stages.iter().enumerate() {
        if let Stage::Wdf(w) = s {
            ...
            eprintln!(
                "  stage[{i}]: Wdf root={root_tag} feedback_opamp={} feedback_pot={:?}",
                w.feedback_opamp.is_some(),
                w.feedback_pot_id,
            );
        } else {
...
/// After updating the Distortion pot on RATKING, the opamp gain should change.
/// On the SPQR-tree branch, RATKING may use BlackFeedback instead of WDF feedback_opamp.
#[test]
fn feedback_opamp_notify_pot_changed_updates_gain() {
    let mut proc = compile_pedal_from_src(RATKING_SRC, SAMPLE_RATE);

    // Try WDF feedback_opamp path first
    let wdf_idx = proc
        .stages
        .iter()
        .position(|s| s.as_wdf().map_or(false, |w| w.feedback_opamp.is_some()));

    // Try BlackFeedback path as fallback
    let bf_idx = proc
        .stages
        .iter()
        .position(|s| matches!(s, Stage::BlackFeedback(_)));

    if let Some(idx) = wdf_idx {
        // WDF feedback_opamp path
        ...
        eprintln!(
            "[notify_pot_changed:wdf] initial_gain={initial_gain:.4}, final_gain={final_gain:.4}"
        );
        ...
        let final_gain = proc.stages[idx].as_black_feedback().unwrap().gain();

        eprintln!(
            "[notify_pot_changed:bf] initial_gain={initial_gain:.4}, final_gain={final_gain:.4}"
        );

        if (final_gain - initial_gain).abs() > 0.01 {
            eprintln!("  -> Gain changed as expected");
        } else {
            eprintln!("  -> Gain did NOT change (pot may not be wired for BlackFeedback path)");
        }
    ...
    for (name, src) in circuits {
        ...
        for (i, s) in proc.stages.iter().enumerate() {
            if let Stage::Wdf(w) = s {
                ...
                if matches!(&w.root, RootKind::OpAmp(_)) {
                    paths.push(format!("stage[{i}]:OpAmpRoot"));
                }
                if w.opamp_adaptor.is_some() {
                    paths.push(format!("stage[{i}]:opamp_adaptor"));
                }
                if w.opamp_wdf_adaptor.is_some() {
                    paths.push(format!("stage[{i}]:opamp_wdf_adaptor"));
                }
            } else if let Stage::BlackFeedback(_) = s {
                paths.push(format!("stage[{i}]:BlackFeedback"));
            }
        }
        eprintln!("[mna_discovery] {name}: paths={paths:?}");
    }

    // Verify at least RATKING uses feedback_opamp or BlackFeedback (current behavior)
...
fn document_opamp_path_routing() {
    /// Collect opamp path labels from a compiled pedal's stages.
    fn collect_opamp_paths(proc: &super::compiled::CompiledPedal) -> Vec<String> {
        let mut paths = Vec::new();
        for (i, s) in proc.stages.iter().enumerate() {
            match s {
                Stage::Wdf(w) => {
    ...
    // Inverting amp (simple resistive feedback)
    {
        ...
        let paths = collect_opamp_paths(&proc);
        let has_opamp_root = wdf_stages(&proc).any(|w| matches!(&w.root, RootKind::OpAmp(_)));
        ...
        eprintln!(
            "[route_inverting] opamp_root={has_opamp_root} opamp_adaptor={has_opamp_adaptor} \
             ...
             black_feedback={has_black_feedback} paths={paths:?}"
        );
    }
