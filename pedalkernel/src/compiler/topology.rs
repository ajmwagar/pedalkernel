    ...
    if (pos_directly_ac_ground || pos_grounded_via_resistor) && !neg_has_independent_ground_leg {
        if let Some((ri, ri_pot_info)) = ctx.find_input_resistor_with_pot(neg_node, out_node) {
            ...
            if feedback_diode.is_none() {
                ...
                let pot_at_out =
                    if let Some(out_neighbors) = ctx.resolved.resistor_adj.get(&out_node) {
                        out_neighbors
                            .iter()
                            .find_map(|(_next, _r, id, is_pot, max_r)| {
                                if !is_pot {
                                    return None;
                                }
                                let wb_id = id;
                                if !wb_id.ends_with("__wb") {
                                    return None;
                                }
                                if !all_fb_comps.contains(wb_id) {
                                    return None;
                                }
