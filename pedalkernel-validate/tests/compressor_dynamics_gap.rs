...
fn circuit_path(suite: &str, name: &str) -> PathBuf {
    validate_dir().join("circuits").join(suite).join(name)
}
...
fn gain_curve_from_golden(golden: &[f64], _freq_hz: f64) -> Vec<(f64, f64)> {
    ...
    for (i, &in_db) in levels_dbvu.iter().enumerate() {
        ...
        let out_rms = rms(seg);
        let out_db = if out_rms > 1e-30 {
            db(out_rms * std::f64::consts::SQRT_2)
        } else {
            -200.0
        };
...
fn envelope_settle_seconds(output: &[f64], step_sample: usize) -> f64 {
    ...
    if step_sample >= output.len() || window_samples == 0 {
        return 0.0;
    }
    let seg = &output[step_sample..];
    let env: Vec<f64> = seg.windows(window_samples).map(|w| rms(w)).collect();
    if env.is_empty() {
        return 0.0;
    }
...
fn fet_leveler_dynamics_gap_report() {
    ...
    let levels_dbvu: Vec<f64> = (-40..=0).step_by(5).map(|l| l as f64).collect();
    let wdf_curve: Vec<(f64, f64)> = levels_dbvu
        .iter()
        .map(|&in_db| {
    ...
    let ngspice_curve: Vec<(f64, f64)> = if golden_sweep_path.exists() {
        let golden = npy::read_f64(&golden_sweep_path).expect("Failed to read level_sweep golden");
        eprintln!("\n=== FET Leveler: Static Gain Curve (ngspice) ===");
        let curve = gain_curve_from_golden(&golden, 1000.0);
        for &(in_db, out_db) in &curve {
            eprintln!(
                "  SPICE: in {in_db:6.1} dBVU -> out {out_db:8.3} dBVU (gain {:8.3} dB)",
        ...
        eprintln!(
            "  WARN: level_sweep golden not found at {}",
            golden_sweep_path.display()
        );
        Vec::new()
    };
    ...
    let (ngspice_attack, ngspice_release) = if golden_burst_path.exists() {
        ...
        let golden_burst =
            npy::read_f64(&golden_burst_path).expect("Failed to read tone_burst golden");
        ...
        eprintln!(
            "  WARN: tone_burst golden not found at {}",
            golden_burst_path.display()
        );
        (f64::NAN, f64::NAN)
    };
    ...
    eprintln!("╠══════════════════════════════════════════════════════╣");
    eprintln!(
        "║ GR (-40 vs 0 dBVU) │ {:>12.3} dB │ {:>12.3} dB ║",
        wdf_gr, ngspice_gr
    );
    eprintln!(
        "║ Ratio (-20..0 dBVU)│ {:>12.4}:1 │ {:>12.4}:1 ║",
        wdf_ratio, ngspice_ratio
    );
    eprintln!(
        "║ Attack (ms)        │ {:>12.1}    │ {:>12.1}    ║",
        wdf_attack * 1000.0,
        ngspice_attack * 1000.0
    );
    eprintln!(
        "║ Release (ms)       │ {:>12.1}    │ {:>12.1}    ║",
        wdf_release * 1000.0,
        ngspice_release * 1000.0
    );
    eprintln!("╚══════════════════════════════════════════════════════╝");
