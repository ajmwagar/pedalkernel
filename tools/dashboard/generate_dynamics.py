...
def plot_static_curve(circuit: dict, out_path: Path, title: str) -> None:
    ...
    ax_a.plot(x, sc["spice_output_dbvu"], color=SPICE_COLOR, marker="o",
    ...
    ax_b.plot(x, sc["spice_gr_db"], color=SPICE_COLOR, marker="o",
              markersize=4, linewidth=1.2, label="ngspice", alpha=0.95)
    ...
    annot = (
        ...
        f"ngspice: ratio {_ratio_str(sc.get('spice_ratio'))}, "
...
def plot_tone_burst(circuit: dict, out_path: Path, title: str) -> None:
    ...
    ax.plot(t, tb["spice_gr_db"], color=SPICE_COLOR, linewidth=1.0,
    ...
    # The TRANSIENT SHAPE is the honest takeaway: WDF is flat (no dynamics) while
    # the reference shows level-dependent gain reduction building over the burst.
    annot = (
...
def main() -> int:
    ...
    for key, circuit in sorted(circuits.items()):
        ...
        if circuit.get("static_curve"):
            ...
            print(f"  ✓ {out}")
            n_static += 1
        if circuit.get("tone_burst"):
            out = out_dir / "tone_burst.png"
            plot_tone_burst(circuit, out, title)
            print(f"  ✓ {out}")
            n_burst += 1
