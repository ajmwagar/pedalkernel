...
mod table_tests {
    ...
    fn table_is_built_for_iir_stage_with_pots() {
        ...
        let iir = compiled.stages.iter().find_map(|s| {
            if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir) } else { None }
        });
        assert!(iir.is_some(), "Should have an IIR stage");
        assert!(iir.unwrap().biquad_table.is_some(), "IIR stage should have a biquad table");
    }
    ...
    fn table_has_correct_dimensions() {
        ...
        assert_eq!(table.dim_labels.len(), 2, "Should have 2 dimensions (2 pots)");
        assert!(table.steps >= 8, "Should have at least 8 steps, got {}", table.steps);

        let expected_entries = table.steps.pow(table.dim_labels.len() as u32);
        assert_eq!(
            table.coeffs.len(), expected_entries * 5,
            "Expected {} coeffs, got {}", expected_entries * 5, table.coeffs.len()
        );
    }
    ...
    fn table_midpoint_matches_direct_iir() {
        ...
        eprintln!("  Table@0.5: b=[{:.6}, {:.6}, {:.6}] a=[{:.6}, {:.6}]",
            coeffs[0], coeffs[1], coeffs[2], coeffs[3], coeffs[4]);
        eprintln!("  Direct:    b=[{:.6}, {:.6}, {:.6}] a=[{:.6}, {:.6}]",
    ...
    fn table_varies_with_cutoff_position() {
        ...
        let cut_dim = table.dim_labels.iter().position(|l| l == "Cutoff_R3").unwrap();
        ...
        eprintln!("  Table@cut=0.1: b=[{:.6},{:.6},{:.6}] a=[{:.6},{:.6}]",
            c_lo[0], c_lo[1], c_lo[2], c_lo[3], c_lo[4]);
        eprintln!("  Table@cut=0.9: b=[{:.6},{:.6},{:.6}] a=[{:.6},{:.6}]",
    ...
    fn cutoff_changes_response_via_direct_set_pot() {
        ...
        for stage in &mut lo2.stages {
            if let crate::compiler::compiled::Stage::Iir(iir) = stage {
                ...
                eprintln!("  After set_pot(0.9): b={:?} a={:?} dc_gain={dc:.4}",
        ...
        for stage in &mut hi2.stages {
            if let crate::compiler::compiled::Stage::Iir(iir) = stage {
                ...
                let dc = iir.iir.dc_gain();
                eprintln!("  After set_pot(0.1): b={:?} a={:?} dc_gain={dc:.4}",
                    &iir.iir.b_coeffs, &iir.iir.a_coeffs);
            }
        }

        eprintln!("  Direct set_pot Cutoff at 5kHz: lo(R=90k)={lo_5k:.1}dB, hi(R=10k)={hi_5k:.1}dB");
        ...
        let lo_dc = lo3.stages.iter().find_map(|s| {
            if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir.iir.dc_gain()) } else { None }
        }).unwrap();
        let hi_dc = hi3.stages.iter().find_map(|s| {
            if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir.iir.dc_gain()) } else { None }
        }).unwrap();
        assert!((lo_dc - hi_dc).abs() > 1.0,
    ...
    fn resonance_changes_response_at_runtime() {
        ...
        for stage in &mut f2.stages {
            if let crate::compiler::compiled::Stage::Iir(iir) = stage {
                ...
                let pos: Vec<_> = iir.pot_bindings.iter().map(|b| format!("{}={:.3}", b.comp_id, b.position)).collect();
        ...
        let a1_flat = f2.stages.iter().find_map(|s| {
            if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir.iir.a_coeffs[1]) } else { None }
        }).unwrap();
        let a1_res = r2.stages.iter().find_map(|s| {
            if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir.iir.a_coeffs[1]) } else { None }
        }).unwrap();
        ...
        let table = iir.biquad_table.as_ref().unwrap();
        let res_dim = table.dim_labels.iter().position(|l| l == "Resonance").unwrap();
        let n = table.dim_labels.len();
        let mut lo_p = vec![0.5; n]; lo_p[res_dim] = 0.0;
        let mut hi_p = vec![0.5; n]; hi_p[res_dim] = 1.0;
        let mut c_lo = [0.0f64; 5];
        let mut c_hi = [0.0f64; 5];
        table.lookup(&lo_p, &mut c_lo);
        table.lookup(&hi_p, &mut c_hi);
        let a1_diff = (c_lo[3] - c_hi[3]).abs();
        eprintln!("  Raw table: res=0 a1={:.6}, res=1 a1={:.6}, diff={a1_diff:.6}",
            c_lo[3], c_hi[3]);

        assert!(a1_diff > 0.001,
        ...
        let iir_m = manual.stages.iter_mut().find_map(|s| {
            if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir) } else { None }
        }).unwrap();
        // Set Resonance to 0.1
        for b in &mut iir_m.pot_bindings {
            if b.comp_id == "Resonance" { b.position = 0.1; }
        }
    ...
    fn cutoff_sweep_monotonic_at_5khz() {
        ...
        eprintln!("  Sweep: {:?}",
            [0.1, 0.3, 0.5, 0.7, 0.9].iter().zip(gains.iter())
        ...
        for &c in &[0.1, 0.3, 0.5, 0.7, 0.9] {
            ...
            let dc = p.stages.iter().find_map(|s| {
                if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir.iir.dc_gain()) } else { None }
            }).unwrap();
            dc_gains.push(dc);
        }
        eprintln!("  DC gain sweep: {:?}",
            [0.1, 0.3, 0.5, 0.7, 0.9].iter().zip(dc_gains.iter())
                .map(|(c, g)| format!("{c:.1}->{g:.2}"))
                .collect::<Vec<_>>()
        );
        assert!((dc_gains[4] - dc_gains[0]).abs() > 1.0,
            "DC gain should vary: first={:.2}, last={:.2}", dc_gains[0], dc_gains[4]);
    }
    ...
    fn diagnostic_trace_set_control_flow() {
        ...
        let iir = proc.stages.iter().find_map(|s| {
            if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir) } else { None }
        }).unwrap();
        eprintln!("  Initial b: {:?}", &iir.iir.b_coeffs);
        eprintln!("  Initial a: {:?}", &iir.iir.a_coeffs);
        eprintln!("  Pot bindings: {:?}",
            iir.pot_bindings.iter().map(|b| format!("{}@{:.3}", b.comp_id, b.position)).collect::<Vec<_>>());
        ...
        if let Some(ref table) = iir.biquad_table {
            ...
            eprintln!("  Table@mid: b=[{:.6},{:.6},{:.6}] a=[{:.6},{:.6}]",
                c[0], c[1], c[2], c[3], c[4]);

            let zeros = vec![0.0; n];
            table.lookup(&zeros, &mut c);
            eprintln!("  Table@min: b=[{:.6},{:.6},{:.6}] a=[{:.6},{:.6}]",
                c[0], c[1], c[2], c[3], c[4]);

            let ones = vec![1.0; n];
            table.lookup(&ones, &mut c);
            eprintln!("  Table@max: b=[{:.6},{:.6},{:.6}] a=[{:.6},{:.6}]",
                c[0], c[1], c[2], c[3], c[4]);
            ...
            if let Some(rd) = res_dim {
                ...
                table.lookup(&lo_pos, &mut c_lo);
                table.lookup(&hi_pos, &mut c_hi);
                eprintln!("  Res=min: b=[{:.6},{:.6},{:.6}] a=[{:.6},{:.6}]",
                    c_lo[0], c_lo[1], c_lo[2], c_lo[3], c_lo[4]);
                eprintln!("  Res=max: b=[{:.6},{:.6},{:.6}] a=[{:.6},{:.6}]",
                    c_hi[0], c_hi[1], c_hi[2], c_hi[3], c_hi[4]);
                let diff = (0..5).map(|i| (c_lo[i] - c_hi[i]).abs()).sum::<f64>();
        ...
        let iir_after = proc.stages.iter().find_map(|s| {
            if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir) } else { None }
        }).unwrap();
        ...
        eprintln!("    Pot positions: {:?}",
            iir_after.pot_bindings.iter().map(|b| format!("{}@{:.3}", b.comp_id, b.position)).collect::<Vec<_>>());
    ...
    fn table_survives_serialization_roundtrip() {
        ...
        let compiled2: crate::compiler::compiled::CompiledPedal = postcard::from_bytes(&blob2).unwrap();

        let iir1 = compiled.stages.iter().find_map(|s| {
            if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir) } else { None }
        }).unwrap();
        let iir2 = compiled2.stages.iter().find_map(|s| {
            if let crate::compiler::compiled::Stage::Iir(iir) = s { Some(iir) } else { None }
        }).unwrap();
