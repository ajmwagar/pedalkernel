...
/// GREEN since 2026-06-15. The LA-2A Gain pot is a 3-terminal divider whose
/// wiper feeds the high-Z 12AX7 grid, splitting (Gain__aw/Gain__wb) across the
/// WDF GR-attenuator stage and the MultiNL tube-grid stage. The multi-target
/// control binding now detects this straddle topologically (wiper → active
/// grid through the 1M grid-leak) and adds a cross-stage `WiperDivider` at the
/// boundary, so the divided wiper voltage reaches the grid. Small-signal gain
/// now sweeps monotonically: -41.2/-30.8/-24.2/-18.9/-14.3 dB at positions
/// 0.1/0.3/0.5/0.7/0.9 (span +26.8 dB). (Was flat -11.8 dB, slightly
